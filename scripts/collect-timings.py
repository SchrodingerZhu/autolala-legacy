...
Every measurement is single-threaded: each job is pinned to its own core with
`taskset`, run at nice 19, with RAYON_NUM_THREADS=1 so `compute_assoc`, which is
rayon-parallel by default, is timed sequentially too. Only the collection runs
48-wide; no measurement ever shares a core.
...
CREATE TABLE IF NOT EXISTS timings (
    ...
    phase_mrc_s        REAL,
    PRIMARY KEY (category, kernel, step, threads, chunk, block_size)
);
"""


def timed(command: list[str], core: int, budget: float) -> tuple[float, str, str, str | None]:
    """Runs one pinned, niced, single-threaded command: (seconds, status, stdout, detail)."""
    wrapped = ["taskset", "-c", str(core), "nice", "-n", "19", *command]
    env = dict(os.environ, RAYON_NUM_THREADS="1", SYMBOLICA_HIDE_BANNER="1")
        ...
        with lock:
            for row in rows:
                connection.execute(
                    ...
                    "(:category,:kernel,:step,:threads,:chunk,:block_size,:associativity,"
                    " :seconds,:status,:detail,:phase_derivation_s,:phase_scaling_s,:phase_mrc_s)",
